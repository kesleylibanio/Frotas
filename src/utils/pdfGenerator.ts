...
export const generateMaintenancePDF = (
  ...
  if (currentServicesNames.includes('Troca de óleo')) {
    ...
    const oilChangeMaintenances = allMaintenances
      ...
      .filter(m => JSON.parse(m.services || '[]').some((s: any) => (typeof s === 'object' ? s.name : s) === 'Troca de óleo'))
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    lastOilChangeMaintenance = oilChangeMaintenances[0];
  }
  
  const lastOilChangeKM = lastOilChangeMaintenance ? lastOilChangeMaintenance.km.toLocaleString() : 'Não informado';
  const lastOilChangeDate = lastOilChangeMaintenance ? new Date(lastOilChangeMaintenance.date).toLocaleDateString('pt-BR') : 'Não informado';

  const interval = intervals.find(i => i.service_type === 'Troca de óleo' && (!i.brand || i.brand === vehicle?.brand) && (!i.vehicle_type || i.vehicle_type === vehicle?.type));
  const nextChangeKM = lastOilChangeMaintenance && interval ? (lastOilChangeMaintenance.km + interval.interval_km).toLocaleString() : 'Não informado';
